import { VALID_DATA_TYPES_INTERNAL } from "./core.js";
...
export function tokenizeStructDef(def) {
...
export function createVecDef(tokens, structDef, { lang, pathToLib, className, exportSyntax, runtimeCompile }) {
    const { elementSize, fieldNames, numberFields, booleanFields, charFields } = tokens;
...
${exportSyntax === "named" ? "export " : ""}class ${className} extends Vec${ts ? generic : ""} {
    ${ts ? "protected " : ""}static Cursor = class Cursor {
        ...
        ${charFields.map(({ field, offset }) => {
            ...
            const getter = `get ${field}()${type} { return String.fromCodePoint(${base} || ${32}) }`;
            const setter = `set ${field}(newValue${type}) { ${base} = newValue.codePointAt(0) || ${32} }`;
            return `${getter}; ${setter};`;
        }).join("\n\t    ")}
