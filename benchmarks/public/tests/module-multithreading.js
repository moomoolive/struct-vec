...
const numberOfCores = 4
const vecWorkers = []
for (let i = 0; i < numberOfCores; i++) {
    vecWorkers.push(new Worker(
        new URL("./tests/workers-module/vecIterModule.js", import.meta.url).href,
        {type: "module"}
    ))
}
...
for (let i = 0; i < numberOfCores; i++) {
    arrWorkers.push(new Worker(
        new URL("./tests/workers-module/arrayIterModule.js", import.meta.url).href,
        {type: "module"}
    ))
}
